...
contract Sniper is Ownable {
    ...
    function snipe(
        ...
        if (pairAddress == address(0)) revert Sniper_INVALIDADDRESS();
        ...
        (uint256 _amount0Out, uint256 _amount1Out) = _tokenIn == token0
            ? (uint256(0), amountOut)
            : (amountOut, uint256(0));
        // TODO: Call internal _swapWithRetreis function with the relevant parameters
        _swapWithRetries(pairAddress, _amount0Out, _amount1Out, _maxRetries, 0);
    ...
    function _swapWithRetries(
        ...
        if (_retryNo >= _maxRetries) revert("Max reached");
