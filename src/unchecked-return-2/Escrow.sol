...
contract Escrow is Ownable {
    ...
    constructor(address _escrowNftAddress) Ownable(msg.sender) {
    ...
    function escrowEth(address _recipient, uint256 _duration) external payable {
        ...
        // @audit no unchecked return and mint function doesn't put consider a contract
        (bool success, ) = escrowNFT.call(
    ...
    function redeemEthFromEscrow(uint256 _tokenId) external {
        ...
        (bool win, ) = escrowNFT.call(
            abi.encodeWithSignature("burn(uint256)", _tokenId)
        );
        if (!win) revert("Failed Transaction");
